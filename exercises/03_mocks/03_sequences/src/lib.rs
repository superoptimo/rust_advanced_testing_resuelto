...
mod tests {
    ...
    fn it_retries_if_first_call_fails() {
        ...
        mock_client.expect_call().times(1).in_sequence(&mut seq).returning(|_s| {
            let errmsg : Box<dyn Error> = anyhow::anyhow!("Error").into();
            Err(errmsg)
        });
    ...
    fn it_does_max_retries_if_all_calls_fail() {
        let mut mock_client = MockClient::new();
        //let mut seq = Sequence::new();
        mock_client.expect_call().times(MAX_N_RETRIES + 1).returning(|_s| {
            let errmsg : Box<dyn Error> = anyhow::anyhow!("Error").into();
            Err(errmsg)
        });
